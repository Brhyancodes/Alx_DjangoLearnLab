...
from .models import Library, Book, CustomUser  # Add CustomUser import
...
# Updated Registration view for user authentication
def register(request):
    if request.method == "POST":
        form = CustomUserCreationForm(
            request.POST, request.FILES
        )  # Include FILES for image upload
        if form.is_valid():
            user = form.save()
            login(request, user)
            return redirect(
...
# Add a user profile view to display custom fields
@login_required
...
# Add a view to edit user profile
@login_required
def edit_profile(request):
    ...
    return render(request, "relationship_app/edit_profile.html", {"user": request.user})
...
# Role-based access control function (updated to work with CustomUser)
